...
def motor_baremacion_itacyl(row):
    """
    MOTOR DE BAREMACIÓN AGRONÓMICA - Versión 1.4.8
    Filtro: diluted=True -> [C,R] y Match Herogra 9,5
    """
    # 0. LIMPIEZA DE DATOS
    def to_float(val):
    ...
    p2o5 = to_float(row.get('p2o5'))
    k2o = to_float(row.get('k2o'))
    ...
    # 2. CLASIFICACIÓN TÉCNICA (Mejorada con la variable 'diluted')
    es_cob = row.get('topDressing') is True
    # Nueva condición: es riego si es líquido, tiene el keyword SOLUB o diluted es True
    ...
    # 3. BAREMO
    # EXCELENCIA (10,0): Rhizovit, Excelis e Inhibidores
    ...
    # Match si el nombre contiene las claves o la combinación TECNOPLUS + CALCIO
    es_nitrato_calcio = any(k in nombre for k in ["NITRATO DE CALCIO", "SOLUTECK", "CALCINIT"]) or ("TECNOPLUS" in nombre and "CALCIO" in nombre)
